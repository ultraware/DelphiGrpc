...
uses
  Classes
{$IFDEF VER210}
  ,System.Generics.Collections, RTTI, TypInfo
{$ENDIF}
  ;
...
{$IFDEF VER210}
  ...
  public
    ...
    constructor Create; virtual;
    destructor Destroy; override;
    function FromJson(TypeInfo: PTypeInfo; const obj: ISuperObject; var Value: TValue; OmitEmptyVariants: Boolean = False): Boolean; virtual;
    function OmitEmptyVariantsOfValue(var TempValue: TValue; OmitEmptyVariants: Boolean): Boolean;
    function ToJson(var value: TValue; const index: ISuperObject; OmitEmptyVariants: Boolean = False): ISuperObject; virtual;
    function AsType<T>(const obj: ISuperObject): T;
    function AsJson<T>(const obj: T; const index: ISuperObject = nil): ISuperObject;
  public
    const SupportsProperties: boolean = True;
  end;

  TSuperObjectHelper = class helper for TObject
  public
    function ToJson(ctx: TSuperRttiContext = nil; OmitEmptyVariants: Boolean = False): ISuperObject;
    function ToOmittedVariantJson(ctx: TSuperRttiContext = nil): ISuperObject;
    constructor FromJson(const obj: ISuperObject; ctx: TSuperRttiContext = nil; OmitEmptyVariants: Boolean = False); overload;
    constructor FromJson(const str: string; ctx: TSuperRttiContext = nil); overload;
    constructor FromOmittedJson(const str: string; ctx: TSuperRttiContext = nil); overload;
  end;
{$ENDIF}
...
//check of onze variant fix aanwezig is
const
  C_SuperObject_Ultraware_Variant_Fix = True;
  C_AUTO_CREATE_SUBOBJECTS = True;
...
begin
  ...
  {$ENDIF}
      ...
      stDouble:
         begin
           Temp := SOString(gcvt(FO.c_double, 15, fbuffer));
           PosPunt := Pos('.',Temp);
           //xxx. kan niet worden gelezen door de iPhone json deserialiser, xxx.0 wel
           // zelfde voor x.E-2
           if (PosPunt = Length(Temp)) or (not CharInSet(Temp[PosPunt+1],['0'..'9'])) then // Als er geen getal achter punt staat, dan een 0 eraan plakken
              Temp := LeftStr(Temp, PosPunt)+ '0' + Copy(Temp,PosPunt+1, Length(Temp));
           Result := Append(PSOChar(Temp));
         end;
      stCurrency:
        begin
...
function TSuperArray.ToJson: string;
var
  i: Integer;
  o: ISuperObject;
begin
...
var
  v: TValue;
begin
//  TValue.MakeWithoutCopy(@obj, TypeInfo(T), v);    // Mem leak ???
// TODO: ALEX?  TValue.Make(@obj, TypeInfo(T), v);
...
function TSuperRttiContext.OmitEmptyVariantsOfValue(var TempValue: TValue; OmitEmptyVariants: Boolean): Boolean;

   function OmitVarRecord(var RecordObj: TValue): Boolean;    // LET OP ! Hele record teruggeven, of niets (geen halve omit binnen record)
     var f: TRttiField;
         v: TValue;
   begin
      Result := True;
      for f in Context.GetType(RecordObj.TypeInfo).GetFields do
      begin
        v := f.GetValue(IValueData(TValueData(RecordObj).FValueData).GetReferenceToRawData);
        if not OmitEmptyVariantsOfValue(v, OmitEmptyVariants) then
           Result := False;
      end;
   end;

var Serial: TSerialFromJson;
begin
    Result := False;
   //    TODO('Uitbreiden, mogelijk per type waarde kunnen uitsluiten, etc ?');

   // We halen property weg als...
    if OmitEmptyVariants and not SerialFromJson.TryGetValue(TempValue.TypeInfo, Serial) then
    begin
       case TempValue.Kind of
   //         tkInt64:                                     Result := (SuperInt(TempValue.AsInt64) = 0);
   //         tkChar:                                      Result := (string(TempValue.AsType<AnsiChar>) = '');
   //         tkSet, tkInteger, tkEnumeration:             Result := (TValueData(TempValue).FAsSLong = 0);
   //         tkFloat:
   //         begin
   //             case Value.TypeData.FloatType of
   //               ftSingle:                              Result := (TValueData(TempValue).FAsSingle = 0);
   //               ftDouble:                              Result := (TValueData(TempValue).FAsDouble = 0);
   //               ftExtended:                            Result := (TValueData(TempValue).FAsExtended = 0);
   //               ftComp:                                Result := (TValueData(TempValue).FAsSInt64 = 0);
   //               ftCurr:                                Result := (TValueData(TempValue).FAsCurr = 0);
   //             end;
   //         end;
   //         tkString, tkLString, tkUString, tkWString:   Result := (string(TempValue.AsType<string>) = '');
   //   //      tkClass: ToClass;
   //         tkWChar:                                     Result := (string(TempValue.AsType<WideChar>) = '');
         tkVariant:                                   Result := VarIsEmpty(TempValue.AsVariant) or VarIsNull(TempValue.AsVariant);
         tkRecord:                                    Result := OmitVarRecord(TempValue);
   //         tkArray: ToArray;
   //         tkDynArray:                                  Result := (TempValue.GetArrayLength = 0);
...
function TSuperRttiContext.FromJson(TypeInfo: PTypeInfo; const obj: ISuperObject; var Value: TValue; OmitEmptyVariants: Boolean = False): Boolean;
  ...
  begin
    ...
    stInt, stBoolean:
      begin
        ...
        Result := (i >= TypeData.MinValue) and (Cardinal(i) <= Cardinal(TypeData.MaxValue)); //-1?
        if Result then
        ...
        begin
          ...
          for f in Context.GetType(Value.AsObject.ClassType).GetFields do
            ...
            begin
              ...
              vcurrent := f.GetValue(Value.AsObject);
              sfield   := GetFieldName(f);
              jnew     := obj.AsObject[sfield];
              if not C_AUTO_CREATE_SUBOBJECTS then
                Assert(false);

              if (jnew <> nil) and (vcurrent.Kind <> tkDynArray) and
                 (vcurrent.IsObject) and (vcurrent.AsObject = nil)
              then
                vcurrent := GetTypeData(vcurrent.TypeInfo).ClassType.Create;

              //load (sub) value
                 Result := FromJson(f.FieldType.Handle, GetFieldDefault(f, jnew), vcurrent, OmitEmptyVariants);

              //write value in object
              if Result then
                f.SetValue(Value.AsObject, vcurrent)
              else
                Exit;
            end;
        end;
  ...
  begin
    ...
    begin
      ...
      begin
         Result := FromJson(f.FieldType.Handle, GetFieldDefault(f, obj.AsObject[GetFieldName(f)]), v, OmitEmptyVariants);
         if Result and not OmitEmptyVariantsOfValue(v, OmitEmptyVariants) then
            f.SetValue(p, v)
         else
            Exit;
      end
      ...
      begin
        Result := OmitEmptyVariantsOfValue(v, OmitEmptyVariants);    //null = ok
        Exit;
      end;
    end;
  end;
  ...
  begin
    ...
    stArray:
      begin
        ...
        begin
          ...
          Result := FromJson(el, obj.AsArray[i], val, OmitEmptyVariants);
          if not Result then
            Break;
    ...
    stNull:
      ...
      Result := FromJson(el, obj, val, OmitEmptyVariants);
      val.ExtractRawData(pb);
      val := TValue.Empty;
    ...
    begin
      ...
      begin
        ...
        if dim = ArrayData.DimCount then
          ...
          begin
            Result := FromJson(ArrayData.ElType^, o.AsArray[i], v, OmitEmptyVariants);
            if not Result then
              Exit;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          Result := FromJson(ArrayData.ElType^, obj.AsArray[i], v, OmitEmptyVariants);
          if not Result then
            Exit;
          Value.SetArrayElement(idx, v);
          v := TValue.Empty;
  ...
  //C_SuperObject_Ultraware_Variant_Fix
  ...
  begin
    case ObjectGetType(obj) of
      ...
      stNull:
        begin
          Value  := TValue.FromVariant(Null);    //wordt helaas nog wel "unassigned" als resultaat :(
          Result := True;
        end
    else
      Value := nil;
  ...
  var
    o: ISuperObject;
  begin
    if CompareMem(@GetTypeData(TypeInfo)
                  {$IF CompilerVersion < 32}
                  .{intf}Guid,
                  {$ELSE}
                  .intfGuid,
                  {$ENDIF}
                  @soguid, SizeOf(TGUID)) then
    begin
      if obj <> nil then
...
function TSuperRttiContext.ToJson(var value: TValue; const index: ISuperObject; OmitEmptyVariants: Boolean = False): ISuperObject;
...
constructor TSuperObjectHelper.FromJson(const obj: ISuperObject; ctx: TSuperRttiContext = nil; OmitEmptyVariants: Boolean = False);
var
  v: TValue;
  ctxowned: Boolean;
begin
  ...
  try
    v := Self;
    if not ctx.FromJson(v.TypeInfo, obj, v, OmitEmptyVariants) then
...
begin
  FromJson(SO(str), ctx, False);
end;

constructor TSuperObjectHelper.FromOmittedJson(const str: string; ctx: TSuperRttiContext = nil);
begin
  FromJson(SO(str), ctx, True);
end;

function TSuperObjectHelper.ToOmittedVariantJson(ctx: TSuperRttiContext = nil): ISuperObject;
begin
   Result := ToJson(ctx, True);
end;

function TSuperObjectHelper.ToJson(ctx: TSuperRttiContext = nil; OmitEmptyVariants: Boolean = False): ISuperObject;
var v: TValue;
  ctxowned: boolean;
begin
  if ctx = nil then
  ...
  try
    v := Self;
    Result := ctx.ToJson(v, SO, OmitEmptyVariants);
  finally
    if ctxowned then
      ctx.Free;
  end;
