...
function TSuperRttiContext.FromJson(TypeInfo: PTypeInfo; const obj: ISuperObject;
        ...
        begin
          ...
          for f in Context.GetType(Value.AsObject.ClassType).GetFields do
            ...
            begin
              ...
              //load (sub) value
              Result := FromJson(f.FieldType.Handle, GetFieldDefault(f, jnew), vcurrent);
  ...
  var
    o: ISuperObject;
  begin
    if CompareMem(@GetTypeData(TypeInfo).{$IF CompilerVersion >= 32}IntfGuid{$else}Guid{$endif}, @soguid, SizeOf(TGUID)) then
    begin
      if obj <> nil then
      ...
      begin
        ...
        for p in Context.GetType(Value.AsObject.ClassType).GetProperties do
          ...
          begin
            ...
            //Result.AsObject[GetFieldName(p)] := ToJson(v, index);
